...
export default (settings = {}) => {
  ...
  function animation(animations) {
    ...
    // const current = animations[Object.keys(animations)[0]];

    return {
      animation: {
        animations,
        current: Object.keys(animations)[0],
        timer: timer(),
        play(name) {
  ...
  const update = (dt) => {
    ...
    sprites.forEach((sprite) => {
      if (sprite.status === 'added') {
        ...
        sprite.animation.timer.set(sprite.animation.animations[sprite.animation.current].delay);
      ...
      if (sprite.animation.timer.expired()) {
        const current = sprite.animation.animations[sprite.animation.current];

        if (sprite.tags.includes('playerMissleWeapon')) console.log(current.frame);
        ...
        if (
          ...
          if (current.repeat)
            ...
            current.onComplete &&
              current.onComplete(sprite);
        }

        sprite.animation.timer.set(current.delay);
      }
  ...
  const draw = () => {
    ...
    sprites.forEach((sprite) => {
      ...
      const frame =
        sprite.animation.animations[sprite.animation.current].sequence[sprite.animation.animations[sprite.animation.current].frame];
