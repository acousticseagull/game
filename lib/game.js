...
export default (settings = {}) => {
  ...
  const update = (dt) => {
    ...
    sprites.forEach((sprite) => {
      ...
      if (sprite.tags.includes('playerMissleWeapon')) {
        //console.log(sprite.animation.animations[sprite.animation.current].delay)
      }

      // animate
      if (sprite.animation.timer.expired()) {
        const { current, animations, timer } = sprite.animation;
